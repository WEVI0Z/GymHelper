...
begin
  ...
  begin
    ...
    names[i].Top := 40 * (i);
    names[i].Left := 5;
    names[i].Caption := 'Упражнение - ' + users[UserID].Stats[CurrentID].Exercises[i].Name;
    names[i].Width := 200;
    names[i].Height := 30;
    ...
    times[i].Top := 40 * (i) + 15;
    times[i].Left := 20;
    times[i].Caption := intToStr(users[UserID].Stats[CurrentID].Exercises[i].Times) + ' раз';
    times[i].Width := 200;
    times[i].Height := 30;
    ...
    begin
      ...
      weights[i].Left := 80;
      weights[i].Caption := intToStr(users[UserID].Stats[CurrentID].Exercises[i].Weight) + ' кг';
      weights[i].Width := 200;
      weights[i].Height := 30;
